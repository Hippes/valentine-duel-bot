...
async def cmd_start(message: Message, state: FSMContext, session: AsyncSession):
    ...
    deep_link_payload = args[0] if args else None
    ...
    if not user.privacy_accepted:
        # Show privacy policy
        # Store deep link payload for later
        if deep_link_payload:
            await state.update_data(deep_link_payload=deep_link_payload)
        ...
        # Check if this is a duel invitation deep link
        if deep_link_payload and deep_link_payload.startswith("duel_"):
            inviter_username = deep_link_payload.replace("duel_", "")
            await message.answer(
                f"🎮 <b>Вас пригласили на дуэль!</b>\n\n"
                f"@{inviter_username} хочет сразиться с вами!\n\n"
                f"Сначала заполните анкету, потом введите ник соперника: <b>@{inviter_username}</b>\n\n"
                f"После этого произойдет матчинг! 🎯",
...
async def accept_privacy(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    # Get deep link payload if exists
    ...
    message_text = "✅ Спасибо! Политика конфиденциальности принята.\n\n"
    
    if deep_link_payload and deep_link_payload.startswith("duel_"):
        inviter_username = deep_link_payload.replace("duel_", "")
        message_text += (
            f"🎮 Вас пригласил на дуэль @{inviter_username}!\n\n"
            f"Сначала заполните анкету, потом введите его ник для матчинга! 📝"
        )
    else:
        message_text += "Теперь давайте заполним вашу анкету! 📝"
