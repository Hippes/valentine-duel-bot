...
class Duel(Base):
    ...
    user2_id: Mapped[Optional[int]] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=True)  # Nullable for pending status
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="pending")  # pending, matched, active, completed, cancelled
    selected_questions: Mapped[Optional[List[int]]] = mapped_column(JSON, nullable=True)  # List of 5 question IDs
    ...
    user2: Mapped[Optional["User"]] = relationship(foreign_keys=[user2_id], back_populates="duels_as_user2")
